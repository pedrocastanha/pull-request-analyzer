...
async def analyze_modules_hierarchically(state: HierarchicalPRAnalysisState) -> Dict:
    ...
    for module_name, files in state["modules"].items():
        logger.info(
            f"[NODE: hierarchical_analysis] Analyzing module '{module_name}' ({len(files)} files)"
        )
        ...
        try:
            ...
            logger.info(
                f"[NODE: hierarchical_analysis] ✓ Module '{module_name}' complete: {total_issues} total issues"
            )

        except Exception as e:
            logger.error(
                f"[NODE: hierarchical_analysis] Error analyzing module '{module_name}': {e}"
            )
            results.append(
                {
                    "module_name": module_name,
    ...
    logger.info(
        f"[NODE: hierarchical_analysis] ✓ All modules analyzed. Total: {len(results)} modules"
    )
