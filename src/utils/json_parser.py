...
def parse_llm_json_response(response_text: str) -> Dict[str, Any]:
    try:
        ...
        logger.debug(f"Successfully parsed JSON with keys: {result.keys() if isinstance(result, dict) else type(result)}")
        return result

    except json.JSONDecodeError as e:
        logger.warning(f"Failed to parse JSON: {str(e)}")
        logger.debug(f"Problematic content (first 1000 chars): {clean_json[:1000] if clean_json else response_text[:1000]}")
